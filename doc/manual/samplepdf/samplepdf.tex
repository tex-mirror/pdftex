...
% pdfTeX 1.50.0 comes with a pretty simple and powerful page tree control mechanism.
% PDF document usually appears to the user as a flat structure of pages in ascending order.
% However, internally, pages are arranged in a tree structure. With \pdfpagedivert<integer>
% command we can specify to which page branch the current page should belong to. 
% With \pdfpageundivert<integer> we append a given branch of pages to
% to the page structure root. pdfTeX itself uses page diversion 0.

\feature 1.50.0 {Page diversion control}

First page diversion\pdfpagedivert 100 % start writing page objects to brach of ID 100
\newpage
Second page diversion\pdfpagedivert 200 % switch to some other branch
\newpage
\pdfpageundivert 200 % flush those pages first
\pdfpageundivert 100 % then those
